...
def plot_spectrogram(title, w, fs, ax, offset=0):
    ...
    snr = np.log10(frequency_response / (energy-frequency_response)) * 10
...
plt.tight_layout()
freq1, FR1, SNR1 = plot_spectrogram('smartphone', smartphone, sr, ax[0], 0.4)
freq2, FR2, SNR2 = plot_spectrogram('stethoscope', stethoscope1, sr, ax[1], 0 )
...
ax[3].plot(freq1, SNR1, label='smartphone')
ax[3].set_title('SNR')
ax[3].plot(freq2, SNR2, label='stethoscope')
ax[3].legend()
